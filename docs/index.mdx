...
The same layer stack is projected onto every pull request. `scripts/pr_decision_diagram.py` classifies a PR's changed paths onto it and draws the layers that moved, so the sticky PR comment answers "which part of the system does this touch" before a reviewer opens a single file.
