...
When a prompt arrives, most of what happens next is **not the model deciding**. A chain of hooks reads the command the model intends to run and passes it, edits it, or blocks it — before it reaches the shell. That chain, the token budget deciding what the model is allowed to see, and the dbt lineage under one reported figure have their own page, with an interactive version of every diagram.

<Card title="The Decision Path" icon="route" href="/decision-path">
  Four requests and their four verdicts, the budget curve that cuts a 176-node search down to eleven, and the 378-model lineage behind a single business number. Every figure measured from this repository, and the artifact-derived ones pinned by `tests/test_decision_path_page.py`.
</Card>

## System architecture

The decision path above answers *how a prompt becomes an action*. This answers *what the thing is*: nineteen source systems, 378 dbt models, 58 conformed concepts, and one semantic layer served to BI and to agents — plus the gates that decide whether any of it can be trusted.

Three lanes, and the arrow between the first two runs **both ways**. Meaning is derived from the warehouse — dbt's own `manifest.json` is the input to every ontology artifact — and the business layer is then generated back *from* that meaning. A conventional layer diagram flattens that into one direction and loses the only structurally interesting thing here.

<iframe
  src="/code-skills-architecture.html"
  title="code-skills system architecture — data flow, derivation pipeline, and layers"
  width="100%"
  height="900"
  loading="lazy"
></iframe>
...
The page is committed at `public/code-skills-architecture.html` and serves at `/code-skills-architecture.html` under Astro's static-asset convention — the same arrangement as the decision path above. It is entirely self-contained, with no external requests, so it works from any path that serves it.
:::

Every figure on that page taken from a committed artifact is checked against that artifact by `tests/test_architecture_diagram.py`, so the picture cannot quietly stop being true. Figures that need a rebuild — the test count, the size of the code graph — are deliberately left unpinned and the page names the command that re-derives each: a gate that goes red because somebody added a test is a gate that gets switched off.

The sticky PR comment `scripts/pr_decision_diagram.py` posts is a different view and no longer a projection of this stack: it draws the *impact subgraph* of the PR's own diff — the code-graph nodes it touches plus one hop either side — because that is the part a reviewer cannot read off the file list.
...
<CardGroup cols={2}>
  ...
  <Card title="The decision path" icon="route" href="/decision-path">
    How a prompt becomes an action: the checkpoints, the token budget, and the lineage under one number.
  </Card>
</CardGroup>
