...
Load the `raw-layer-ontology` skill. Derive with
[scripts/raw_taxonomy.py](../../../scripts/raw_taxonomy.py). The artifacts are
...
If most tables report `declared_columns: 0`, the raw layer has no column contracts and the
ontology would be entities with no attributes. Bootstrap them first:
...
Then write `grain` for every entity — one sentence, "one row per X per Y"
([rule 4](../../rules/analytics-engineering-rules.md)). Nothing derives it. **Do not invent
one to make the file look finished** ([rule 5](../../rules/analytics-engineering-rules.md));
an entity you cannot complete is reported incomplete, and that report is the point.
...
Exit 1 means real gaps: an attribute tracing to no declared column, a natural key no source
has, a missing grain. Fix the taxonomy and re-run until it exits 0.
...
Declared entities with no dbt model yet, each with its grain and sources. Build them one at
a time with [/dbt-model](dbt-model.md) — the grain is already decided, so each model has
one job.

## 5. Report

State: entities declared, raw tables mapped of declared, concepts with no source (`gaps` —
the most useful output, because it says what the data cannot answer yet), and what is still
to build. Name the decisions the user made, so the next session can see that they were
