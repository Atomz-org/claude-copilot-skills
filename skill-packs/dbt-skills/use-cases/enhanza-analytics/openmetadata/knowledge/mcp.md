...
{
  "mcpServers": {
    "openmetadata-enhanza-analytics": {
...
curl -s -H "Authorization: Bearer $OPENMETADATA_AUTH_TOKEN" \
  "$OPENMETADATA_SERVER_URL/api/v1/tables/name/enhanza_bigquery.<db>.<schema>.<table>?fields=columns,tags"
...
# a glossary term
curl -s -H "Authorization: Bearer $OPENMETADATA_AUTH_TOKEN" \
  "$OPENMETADATA_SERVER_URL/api/v1/glossaryTerms/name/enhanza-analytics.<term>"
```
