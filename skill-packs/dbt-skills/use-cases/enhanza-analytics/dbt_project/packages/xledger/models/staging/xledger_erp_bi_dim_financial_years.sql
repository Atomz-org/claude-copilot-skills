...
with min_max1 as (
  select
    OrgId
    , date(date_trunc(min(parse_date('%F', string(json_extract(period, '$.fromDate')))), year)) minDate
    , date(date_trunc(max(parse_date('%F', string(json_extract(period, '$.toDate')))), year)) maxDate
  from {{ source('xledger_api', 'recent_transactions') }}
  group by OrgId
)
, min_max2 as (
  select
    OrgId
    , date(date_trunc(min(parse_date('%F', string(json_extract(header, '$.period.fromDate')))), year)) minDate
    , date(date_trunc(max(parse_date('%F', string(json_extract(header, '$.period.toDate')))), year)) maxDate
  from {{ source('xledger_api', 'transactions') }}
  group by OrgId
)
, min_max as (
  -- Combine the two tables and compute the overall min and max dates
  select
    coalesce(m1.OrgId, m2.OrgId) OrgId,
    least(m1.minDate, m2.minDate) minDate,
    greatest(m1.maxDate, m2.maxDate) maxDate
  from min_max1 m1
  full outer join min_max2 m2 on m1.OrgId = m2.OrgId
)
, define_dates as (
  select
    OrgId
