...
# ---------------------------------------------------------------------------
# Architecture layers — the per-PR projection of the layer stack
# ---------------------------------------------------------------------------

def test_a_use_case_artifact_is_not_mistaken_for_a_harness_asset():
    """The ordering trap: a use-case's dbt project and ontology live *under*
    `skill-packs/`, so a rule list ordered by layer rather than by specificity
    files the whole warehouse as harness."""
    base = "skill-packs/dbt-skills/use-cases/enhanza-analytics"
    assert pdd.classify_layer(f"{base}/dbt_project/models/erp/x.sql") == "artifacts"
    assert pdd.classify_layer(f"{base}/ontology/index.json") == "artifacts"
    assert pdd.classify_layer(f"{base}/wren/models/orders.yml") == "serving"
    assert pdd.classify_layer("skill-packs/dbt-skills/.claude/skills/dbt-skill/SKILL.md") == "harness"


def test_a_rules_file_under_wren_is_serving_not_harness():
    """`/rules/` reads as a harness segment and is not one here: the knowledge a
    WrenAI agent loads at query time is a generated serving artifact."""
    assert pdd.classify_layer("wren/knowledge/rules/general.md") == "serving"
    assert pdd.classify_layer(".claude/rules/wren-rules.md") == "harness"


def test_markdown_is_documentation_only_at_the_repository_root():
    assert pdd.classify_layer("CLAUDE.md") == "docs"
    assert pdd.classify_layer("docs/BRANCHING_STRATEGY.md") == "docs"
    assert pdd.classify_layer("tests/test_x.py") == "verification"
    assert pdd.classify_layer(".github/workflows/ci.yml") == "verification"
    assert pdd.classify_layer("scripts/hooks/toon_graphify_pipe.py") == "harness"
    assert pdd.classify_layer("scripts/ontology_generator.py") == "core"


def test_an_unrecognised_path_is_reported_not_swallowed():
    """`other` is a finding: either a new kind of thing, or a stale rule."""
    assert pdd.classify_layer("vendor/thing.bin") == "other"
    layers = pdd.architecture_layers(["vendor/thing.bin"])
    assert {lay["id"] for lay in layers} >= {"other"}
    assert [lay["count"] for lay in layers if lay["id"] == "other"] == [1]


def test_the_other_bucket_is_absent_when_everything_classified():
    layers = pdd.architecture_layers(["scripts/x.py", "tests/test_x.py"])
    assert "other" not in {lay["id"] for lay in layers}


def test_the_whole_stack_is_drawn_so_untouched_layers_still_read(tmp_path):
    document = _render(tmp_path, changed_files=["scripts/ontology_generator.py"])
    section = document.split("### Architecture layers touched")[1]
    assert "Derivation core<br/>1 file" in section
    assert "Serving<br/>untouched" in section
    assert ":::touched" in section and ":::quiet" in section


def test_two_prs_in_different_layers_render_differently(tmp_path):
    """Same reason the impact subgraph exists: a section identical on every PR
    carries no information."""
    core = _render(tmp_path, changed_files=["scripts/ontology_generator.py"])
    harness = _render(tmp_path, changed_files=[".claude/skills/x/SKILL.md"])
    assert core != harness
    assert "Harness<br/>1 file" in harness
    assert "Harness<br/>untouched" in core


def test_the_section_names_the_page_it_projects(tmp_path):
    document = _render(tmp_path, changed_files=["scripts/x.py"])
    assert pdd.ARCH_DOC in document
    assert (REPO / pdd.ARCH_DOC).is_file(), "the comment links to a file that must exist"


def test_no_changed_files_means_no_section(tmp_path):
    """A workflow run that could not resolve the diff has nothing to project, and
    an empty stack drawn anyway is the fixed diagram this renderer rejected."""
    assert "Architecture layers touched" not in _render(tmp_path)


def test_the_section_survives_a_run_with_no_code_graph(tmp_path):
    """Layers come from the paths alone, so the projection still renders when the
    impact subgraph reports that graph.json was never built."""
    impact = pdd.build_impact(None, ["scripts/x.py"], "")
    document = _render(tmp_path, impact=impact, changed_files=["scripts/x.py"])
    assert not impact["available"]
    assert "Derivation core<br/>1 file" in document


def test_layer_labels_are_escaped_like_every_other_label():
    """Constants today, but they reach a Mermaid label on the same path as the
    attacker-controlled ones, and the next label added may not be a constant."""
    rendered = "\n".join(pdd.render_architecture([{"id": "x", "label": 'a"|<b>', "count": 2}]))
